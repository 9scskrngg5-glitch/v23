      ...
      {stats ? (
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(155px, 1fr))", gap: 8, marginBottom: 16 }}>
            ...
            <StatCard label="Profit Factor" value={stats.profitFactor} color={Number(stats.profitFactor) >= 1.5 ? ${C.green} : Number(stats.profitFactor) >= 1 ? ${C.orange} : ${C.re}d} sub="≥ 1.5 = bon" />
            ...
            <StatCard label="Régime" value={regime ? REGIME_META[regime]?.label : "—"} color={regime ? REGIME_META[regime]?.color : ${C.textDi}m} sub={regime ? REGIME_META[regime]?.hint : "10 trades min"} />
            ...
            <AreaChart data={equity} margin={{ top: 16, right: 0, left: 0, bottom: 0 }}>
              <defs>
                <linearGradient id="eqG" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={${C.gree}n} stopOpacity={0.2} />
                  <stop offset="95%" stopColor={${C.gree}n} stopOpacity={0} />
                </linearGradient>
              </defs>
              <CartesianGrid strokeDasharray="3 3" stroke={${C.borde}r} strokeOpacity={0.4} vertical={false} />
              ...
              <Area type="monotone" dataKey="eq" stroke={${C.gree}n} strokeWidth={2.5} fill="url(#eqG)" dot={false} activeDot={{ r: 5, fill: ${C.green}, strokeWidth: 0 }} />
            </AreaChart>
          </ResponsiveContainer>
        ) : (
