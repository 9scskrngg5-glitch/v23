  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      pixels = Array.from({ length: 120 }, () => ({
        ...
        vy: -(Math.random() * 0.25 + 0.05), vx: (Math.random() - 0.5) * 0.1,
    ...
    const draw = () => {
      ...
      pixels.forEach(p => {
        ...
        const green = Math.random() < 0.15;
        ctx.fillStyle = green ? `rgba(62,207,142,${a * 0.6})` : `rgba(200,210,230,${a})`;
        ctx.fillRect(Math.round(p.x), Math.round(p.y), p.size, p.size);
      });
      // Bottom fog
      const fog = ctx.createLinearGradient(0, H * 0.8, 0, H);
      fog.addColorStop(0, "rgba(8,9,11,0)");
      fog.addColorStop(1, "rgba(8,9,11,0.7)");
      ctx.fillStyle = fog;
...
// Floating stat card
const StatFloat = ({ label, value, color = G.green, delay = "0s", top, left, right, bottom }) => (
  <div style={{
    position: "absolute", top, left, right, bottom,
    background: "rgba(17,19,22,0.85)", backdropFilter: "blur(12px)",
    ...
    <div style={{ fontSize: 16, fontWeight: 300, fontFamily: "'DM Mono',monospace", color, letterSpacing: "-0.02em" }}>{value}</div>
  </div>
);
...
export const AuthScreen = ({ onSignIn, onSignUp, onSignInWithGoogle }) => {
  ...
  const inp = {
    background: G.inner, border: `1px solid ${G.border}`, color: G.text,
    padding: "11px 14px", borderRadius: 9, width: "100%", fontSize: 13,
    fontFamily: "'DM Mono',monospace", outline: "none", transition: "all 0.15s",
  };
    ...
    <div style={{ background: G.bg, minHeight: "100dvh", display: "flex", alignItems: "stretch", fontFamily: "'DM Sans',sans-serif" }}>
      <style>{`
        *,*::before,*::after{box-sizing:border-box}
        ...
        input:focus{border-color:rgba(62,207,142,0.5) !important;box-shadow:0 0 0 3px rgba(62,207,142,0.1) !important}
        @media(max-width:768px){.al{display:none!important}.ar{width:100%!important}}
      `}</style>

      {/* LEFT — illustration */}
      <div className="al" style={{ width: "50%", position: "relative", overflow: "hidden", background: G.bg }}>
        <AnimatedBg />
        {/* Overlay */}
        <div style={{ position: "absolute", inset: 0, background: "radial-gradient(ellipse at 30% 50%, rgba(62,207,142,0.03) 0%, transparent 70%)", zIndex: 1 }} />
        ...
        <div style={{ position: "absolute", top: 28, left: 32, zIndex: 3, display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontSize: 12, color: G.text, fontFamily: "'DM Mono',monospace", letterSpacing: "0.12em" }}>
        ...
        <StatFloat label="PnL Total" value="+$2,840" color={G.text} top="18%" right="8%" delay="1.2s" />
        <StatFloat label="Expectancy" value="+$47.2" top="42%" left="12%" delay="0.6s" />
        <StatFloat label="Drawdown" value="-$340" color={G.red} top="42%" right="10%" delay="1.8s" />

        {/* Bottom content */}
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, zIndex: 3, padding: "0 28px 32px" }}>
          <div style={{ marginBottom: 20 }}>
            <h2 style={{ fontSize: 24, fontWeight: 800, fontFamily: "'Syne',sans-serif", color: G.text, margin: "0 0 6px", letterSpacing: "-0.02em", lineHeight: 1.1 }}>
              Commence à trader<br/>avec discipline.
            </h2>
            <p style={{ fontSize: 11, color: G.dim, fontFamily: "'DM Mono',monospace", lineHeight: 1.6, margin: 0 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {[
              ...
              <div key={s.n} style={{
                ...
                background: s.active ? "rgba(62,207,142,0.12)" : "rgba(255,255,255,0.03)",
                border: `1px solid ${s.active ? G.greenBord : G.border}`,
                borderRadius: 12, backdropFilter: "blur(8px)",
              }}>
                <div style={{
                  width: 20, height: 20, borderRadius: "50%",
                  background: s.active ? G.green : "rgba(255,255,255,0.06)",
                  border: s.active ? "none" : `1px solid ${G.border}`,
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: 10, fontWeight: 700, color: s.active ? G.bg : G.dim,
                  fontFamily: "'DM Mono',monospace", marginBottom: 10,
                }}>{s.n}</div>
                <div style={{
                  fontSize: 11, fontWeight: 500, color: s.active ? G.green : G.mid,
      ...
      <div className="ar af" style={{ width: "50%", display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 24px", background: G.bg }}>
        <div style={{ width: "min(380px,100%)" }}>
          ...
          <div style={{ background: G.card, border: `1px solid ${G.border}`, borderRadius: 18, padding: 26, position: "relative", overflow: "hidden" }}>
            {/* Top shine */}
            ...
            {error && <div style={{ marginTop: 13, background: G.redDim, border: `1px solid ${G.redBord}`, padding: "9px 13px", borderRadius: 8, color: G.red, fontSize: 12, fontFamily: "'DM Mono',monospace" }}>{error}</div>}
            ...
            <button onClick={handleSubmit} disabled={loading} style={{
              marginTop: 17, width: "100%", padding: "12px", borderRadius: 10, border: "none",
              background: loading ? G.inner : G.green, color: loading ? G.dim : G.bg,
              cursor: loading ? "not-allowed" : "pointer", fontSize: 12, fontWeight: 700,
              fontFamily: "'DM Mono',monospace", letterSpacing: "0.1em", transition: "all 0.15s",
              ...
              {loading && <div style={{ width: 12, height: 12, border: `2px solid rgba(8,9,11,0.2)`, borderTop: `2px solid ${G.bg}`, borderRadius: "50%", animation: "spin 0.7s linear infinite" }} />}
