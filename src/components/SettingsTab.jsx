        ...
        <div>
          <div style={{ ...card(), marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 16 }}>
              <button style={btnStyle()} onClick={() => { const blob = new Blob([JSON.stringify({ trades, tasks, version: 1, exportedAt: Date.now() }, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `trading-journal-${new Date().toISOString().slice(0,10)}.json`; a.click(); URL.revokeObjectURL(url); }}>EXPORT JSON</button>
              <button style={btnStyle()} onClick={() => exportTradesToCSV(trades)}>EXPORT CSV</button>
            </div>
            <div style={{ display: "flex", gap: 10, alignItems: "flex-end", flexWrap: "wrap" }}>
              ...
              <button style={btnStyle(isPro ? "primary" : "default")} onClick={handleExportPDF} disabled={exporting}>
                {exporting ? "EXPORT..." : isPro ? "EXPORTER" : "PRO"}
              </button>
            </div>
          </div>
          ...
          <div style={{ ...card() }}>
            ...
            <textarea value={importText} onChange={e => setImportText(e.target.value)}
              ...
              style={{ width: "100%", minHeight: 100, background: C.bgInner, border: `1px solid ${C.border}`, color: C.text, borderRadius: 9, padding: "10px 13px", fontSize: 12, fontFamily: F.mono, outline: "none", resize: "vertical" }}
              onFocus={e => e.target.style.borderColor = C.green} onBlur={e => e.target.style.borderColor = C.border}
            />
            ...
            {importSuccess && <div style={{ marginTop: 10, background: C.greenDim, border: `1px solid ${C.greenBord}`, padding: "10px 13px", borderRadius: 8, color: C.green, fontSize: 12, fontFamily: F.mono }}>Import réussi ✓</div>}
            <div style={{ display: "flex", gap: 8, marginTop: 12, justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ fontSize: 11, color: C.textGhost, fontFamily: F.mono }}>{trades.length} trades · {tasks.length} tâches</div>
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => { setImportText(""); setImportError(""); }} style={btnStyle()}>ANNULER</button>
                <button onClick={handleImport} style={btnStyle("primary")}>IMPORTER</button>
              </div>
            </div>
          </div>
        </div>
