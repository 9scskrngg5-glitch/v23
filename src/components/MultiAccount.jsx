...
export const useMultiAccount = () => {
  const [accounts, setAccounts] = useState(() => {
    try {
      ...
      return saved || [{ id: "default", name: "Compte principal", color: "#00e5a0", active: true }];
    } catch { return [{ id: "default", name: "Compte principal", color: "#00e5a0", active: true }]; }
  });
...
const COLORS = ["#00e5a0", "#f5a623", "#a78bfa", "#60a5fa", "#f87171", "#34d399"];
      ...
      <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
        {accounts.map(a => (
          <button key={a.id} onClick={() => onSwitch(a.id)} style={{
            ...
            borderColor: activeId === a.id ? `${a.color}50` : "#13162a",
            background: activeId === a.id ? `${a.color}0d` : "transparent",
            cursor: "pointer", fontSize: 11, fontFamily: mono,
            color: activeId === a.id ? a.color : "#3a4060",
            transition: "all 0.15s",
          }}>
            <div style={{ width: 6, height: 6, borderRadius: "50%", background: a.color }} />
            {a.name}
            {a.id !== "default" && activeId === a.id && (
              <span onClick={e => { e.stopPropagation(); onRemove(a.id); }} style={{ color: "#ff4d6d", fontSize: 12, marginLeft: 2, cursor: "pointer" }}>×</span>
            )}
          </button>
        ))}
        <button onClick={() => setShowAdd(s => !s)} style={{ padding: "7px 14px", borderRadius: 8, border: "1px solid #13162a", background: "transparent", color: "#2d3352", cursor: "pointer", fontSize: 10, fontFamily: mono }}>
          {showAdd ? "×" : "+"}
        </button>
      </div>

      {showAdd && (
        <div style={{ position: "absolute", top: "calc(100% + 8px)", left: 0, background: "#0a0d18", border: "1px solid #181b2e", borderRadius: 12, padding: 16, zIndex: 50, minWidth: 240, boxShadow: "0 8px 24px rgba(0,0,0,0.4)" }}>
          <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="Nom du compte"
            style={{ background: "#080a14", border: "1px solid #181b2e", color: "#dde1f5", padding: "8px 12px", borderRadius: 8, width: "100%", fontSize: 12, fontFamily: mono, outline: "none", marginBottom: 10 }}
          ...
          <button onClick={handleAdd} style={{ width: "100%", padding: "8px", borderRadius: 8, border: "none", background: "#00e5a0", color: "#000", cursor: "pointer", fontSize: 11, fontWeight: 700, fontFamily: mono, letterSpacing: "0.06em" }}>
            CRÉER
          </button>
        </div>
      )}
